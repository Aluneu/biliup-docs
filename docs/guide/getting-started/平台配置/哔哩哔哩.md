...
* 多数直播分区都是`HLS`流，需要用到`streamlink`下载器，该下载器依赖`ffmpeg`，请确认是否安装，详见[下载器的选择](../配置/global-config.html#下载插件-downloader)       
* 重要功能将使用⭐符号来进行标注，请注意查看。           
:::


::: warning
* 请勿使用网上购买的 cookies「协议号」来进行登录，此类**黑号**缺失必要的`值`。
:::
...
* 该功能在`强制获取真原画`之前生效，若不清楚用途，保持默认即可。
