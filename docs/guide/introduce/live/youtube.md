...
> [!TIP]
> * 推荐使用 `streamlink` 或 `yt-dlp` 下载器以获得更好兼容性，需先完成 [FFmpeg 安装](../../docs/ffmpeg安装.html)。
> * 支持通过 **Yaml 源文件**添加 YouTube 直播源与 VOD，详见[开发者选项](../Config/developerOptions.html)。

----

## 登录信息（Cookie）

* 配置 YouTube 登录信息（Cookie 文件）后可解锁受限内容、提升稳定性。请在[登录方式详解](../introduce/login.html)中了解网页 Cookie 文件登录，并在主播编辑页关联。
